...
export default function Home() {
  ...
  async function buyNft(nft) {
    // Needs the user to sign the transaction, so we will use Web3Provider and sign it
            ...
            nfts.map((nft, i) => (
              <div key={i} className='border shadow rounded-xl overflow-hidden'>
                ...
                <div className='p-4 bg-black'>
                  ...
                  <button className='mt-4 w-full bg-orange-500 text-white font-bold py-2 px-12 rounded' onClick={() => buyNft(nft)}>Buy</button>
                </div>  
              </div>
            ))
          }
